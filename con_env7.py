...
class UR5_robotiq():
    ...
    def step(self, action):
        ...
        current=[]
        for i in range(5):
            init.append(self.init_pose[i]-self.goal_pose[i])
            current.append(self.next_state_dict[i+13]-self.goal_pose[i])
        init=np.asarray(init)
        current=np.asarray(current)
        init_error=np.linalg.norm(init,axis=-1,ord=2)
        joint_error=np.linalg.norm(current,axis=-1,ord=2)
        self.done=False
        info=joint_error
        reward=-joint_error/(init_error*1.2)
        #reward-=(self.next_state_dict[2]/100)/0.12*0.2
        #temp=abs(force/800)
        if joint_error>1.2*init_error:
            self.done=True
            print('out of range')
            #print(dis_error,ori_error)
            reward=-1
        
        if self.next_state_dict[2]<0.005:
            reward=1
            #self.down(0.05)
            print('going in')
            self.done=True
    ...
    def move(self, action,setTime=0.01):
        ...
        jointPos = [state[0] for state in joint_states]
        jointPos=jointPos[1:7]
        for i, name in enumerate(self.controlJoints):
            joint = self.joints[name]

            targetJointPos = jointPos[i]+action[i]*0.001
    ...
    def get_state(self):
        ...
        joint=np.asarray(jp)
        obs=np.concatenate((obs1,joint),axis=None)
    ...
    def loo(self):
        jointPos=[-0.1820821118623146, -0.9890873075052765, 1.821392281664659, -2.4021237312015464, -1.570839270891886, -0.4438456055190232]
        for i, name in enumerate(self.controlJoints):
            joint = self.joints[name]
            targetJointPos = jointPos[i]
    ...
    def lol(self):
        ...
        print(jointPos)
        time.sleep(1000)
        #-0.1820821118623146, -0.9890873075052765, 1.821392281664659, -2.4021237312015464, -1.570839270891886, -0.4438456055190232
