...
class UR5_robotiq():
    ...
    def step(self, action):
        ...
        rel_pose=np.asarray([self.next_state_dict[0],self.next_state_dict[1],self.next_state_dict[2]])
        rel_ori=np.asarray([self.next_state_dict[3],self.next_state_dict[4],self.next_state_dict[5]])
        torque=[]
        for i in range(6):
        ...
        reward=0
        if (dis_error<self.distance_threshold) and (self.done) and (ori_error<0.001):
            goal=True
        if ((dis_error)/self.initdis)>2:
            self.done=True
            print('outofrange')
        #for i in range(6):
        #    if torque[i]>self.thtorque:
        #        reward-=1/6
        if self.done:
            self.contact=False
            if goal:
    ...
    def reset(self):
        ...
        temp=np.asarray([self.state_dict[0],self.state_dict[1],self.state_dict[2]])
    ...
    def move(self, action):
        ...
        for i in range(len(self.controlJoints)):
            if i > 6:
                break
            a_list.append(joint_positions[i+1])
        new_joint_states=[]
        for i in range(6):
            new_joint_states.append(a_list[i]+n_action[i])
        for i, name in enumerate(self.controlJoints):
            joint = self.joints[name]
            targetJointPos = new_joint_states[i]
            #targetJointPos = n_action[i]
            p.setJointMotorControl2(self.robotID,
                                        joint.id,
                                        p.POSITION_CONTROL,
                                        targetPosition = targetJointPos,
    ...
    def get_state(self):
        ...
        self.contact = False
        self.contact1 = p.getContactPoints(bodyA=self.robotID,bodyB=self.boxId,linkIndexA = 7)
        self.contact2 = p.getContactPoints(bodyA=self.robotID,bodyB=self.tableID ,linkIndexA = 7)
        ...
        obs_temp = np.array([
            self.object_rel_pos[0], self.object_rel_pos[1], self.object_rel_pos[2], rel_ori[0],rel_ori[1],rel_ori[2] ,
                    ee_linear_vel[0],ee_linear_vel[1],ee_linear_vel[2],
            ]) #말단과 hole의 x, y위치만 선정
        obs1=np.concatenate((obs_temp,torque),axis=None)
        obs=np.concatenate((obs1,jp),axis=None)
        return obs
